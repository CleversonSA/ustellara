...
struct demod_state
{
	...
	int      post_downsample;
	int      output_scale;
	int      squelch_level, conseq_squelch, squelch_hits, terminate_on_squelch, squelch_timeout, squelch_timeout_lm;
...
struct event_file_log evt_flog;
...
void usage(void)
{
	fprintf(stderr,
		...
		"\t[-e (full event log filename)]\n"
		"\t	Enables a log event file output of rtl_fm activity, like RMS level, squelch hits, center frequency. The file is appendedtime by time.\n"
		"\t[-L (milliseconds)]\n"
		"\t	Enables squelch listen timeout, if you want to scan frequencies, listen a station for X millisecs and scan next one again. The default squelch just stops when a signal strong is found and keep scanning only when the signal is bellow the minimum level. \n"
...
int log_event_watchdog(struct event_file_log *e)
{
	clock_t current_millis = clock();
	int millis_size = LOG_EVENT_WATCHDOG_MS * (CLOCKS_PER_SEC / 1000);
...
int squelch_watchdog(struct demod_state *d)
{
	clock_t current_millis = clock();
	int millis_size = d->squelch_timeout * (CLOCKS_PER_SEC/1000);


	if (d->squelch_timeout_lm == 0) {
		d->squelch_timeout_lm = (int)current_millis;
		return 0;
	}

	if ((int)current_millis > (d->squelch_timeout_lm + millis_size)) {
		d->squelch_timeout_lm = (int)current_millis;
		return 1;
	}
...
void init_log_file(struct event_file_log *eflog)
{
	
	eflog->is_log_enabled = 0;
	eflog->last_millis = 0;
	eflog->logfile = NULL;
	eflog->log_file_full_path = NULL;
}

		
void lprintevtf(struct event_file_log *eflog, char *evt_type, char *evt_data)
{
...
void log_full_demod(struct event_file_log *eflog, char *evt_type, int rms, struct dongle_state *d)
{
	char ebuffer[30];
...
void full_demod(struct demod_state *d)
{
	...
	if (d->squelch_level) {
		...
		if (sr < d->squelch_level) {
			...
			d->squelch_timeout_lm = 0;
			log_full_demod(&evt_flog,"sql_wait",sr,&dongle);
		...
		log_full_demod(&evt_flog,"rms",sr,&dongle); 
	}
...
int main(int argc, char **argv)
{
	...
	init_log_file(&evt_flog);	

	while ((opt = getopt(argc, argv, "e:L:d:f:g:s:b:l:o:t:r:p:E:F:A:M:hT")) != -1) {
		switch (opt) {
		case 'e':
